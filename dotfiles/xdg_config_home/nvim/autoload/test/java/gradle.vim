if !exists('g:test#java#gradle#file_pattern')
    let g:test#java#gradle#file_pattern = '\v([Tt]est.*|.*[Tt]est(s|Case)?)\.java$'
endif

function! test#java#gradle#test_file(file) abort
    return a:file =~? g:test#java#gradle#file_pattern
                \ && exists('g:test#java#runner')
                \ && g:test#java#runner ==# 'gradle'
endfunction

function! test#java#gradle#build_position(type, position) abort
...
function! test#java#gradle#build_args(args) abort
    return a:args
endfunction
